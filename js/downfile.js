...
function DrawImage(ImgD, FitWidth, FitHeight, Hard) { //|| Hard 是否不管三七二十一就調整就對了
    try {
        ...
        if (ImgD != null) {
            ...
            image.onload = function () {
                image.width = ImgD.width != 0 ? ImgD.width : ImgD.naturalWidth;
                image.height = ImgD.height != 0 ? ImgD.height : ImgD.naturalHeight;
                //alert(image.width);
...
(function ($) {
    ...
    $.extend({
        ...
        deleteAttach: function (handlerPath, aid, afileID,fileType, e) {//handlerPath 路徑,aid 為Guid,afileID為ID
            $.ajax({
                async: false,
                type: "POST",
                url: handlerPath,
                ...
                success: function (response) {
                    if (response == "success") {
                        //成功後把<a>的最上面的TR刪除
                        var trID = "tr" + e.getAttribute("afileID"); //抓到那個<tr trID="此值">
                        $("tr[trID='" + trID + "']").remove();
                    }
                    else {
                        alert(response);
